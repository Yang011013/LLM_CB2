...
class GameDisplay(object):
    ...
    def visualize_follower_location_orientation(self, follower_location=None, follower_orientation=None):
        (x, y) = self.transform_to_screen_coords(follower_location.cartesian())
        pygame.draw.circle(
            self._screen, pygame.Color("red"), (x, y), 9
        )
        pygame.draw.line(
            self._screen,
            pygame.Color("red"),
            (x, y),
            (
                x + 20 * math.cos(math.radians(follower_orientation-60)),
                y + 20 * math.sin(math.radians(follower_orientation-60)),
            ),
            4
        )
