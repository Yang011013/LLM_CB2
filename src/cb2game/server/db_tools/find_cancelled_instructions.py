...
from cb2game.server.config import config
from cb2game.server.messages.logs import Direction
from cb2game.server.messages.message_from_server import MessageType
from cb2game.server.schemas import base
from cb2game.server.schemas.game import Instruction
...
def main(
    config_filepath="config/separated-games-2.json",
            ...
            if message["type"] == MessageType.OBJECTIVE.value:
                for objective in message["objectives"]:
                    if objective["cancelled"]:
                        if turn_number == -1:
                            logger.warning(
                                f"Found cancelled objective before game state. Game {game.id}"
                            )
                            continue
                        instruction = (
                            Instruction.select()
                            .where(Instruction.uuid == objective["uuid"])
                            .get()
                        )
                        if instruction.turn_completed != -1:
                            logger.warning(
                                f"Found cancelled objective that was already completed. Game {game.id} instruction {instruction.uuid}"
                            )
                            instruction.turn_cancelled = -1
                            continue
                        if instruction.turn_cancelled != -1:
                            ...
                            logger.warning(
                                f"Found cancelled objective that was already cancelled. Game {game.id} instruction {instruction.uuid}"
                            )
                            continue
