...
import cb2game.server.messages.turn_state as turn_msg
import cb2game.server.schemas.defaults as defaults
import cb2game.server.schemas.util as schema_util
...
from cb2game.server.schemas.mturk import Assignment, Worker, WorkerExperience
from cb2game.server.schemas.prop import PropUpdate
...
def migrate_to_new_game(
    ...
    if len(initial_states) == 0:
        ...
        if len(leader_move) == 0 or len(follower_move) == 0:
            logger.warning(f"Unable to recover initial state for game {old_game.id}")
