...
def DescribeLocationFromActor(location: HecsCoord, follower_location, follower_orientation, map, cards) -> str:
    ...
    if instruction == default_instruction:
        ...
        # return f"Path not in sight. {DescribeBearingFromActor(location, follower_location, follower_orientation)}"
    return f"Path to reach: {instruction}"
def DescribeMap(
    ...
    for lake in metadata.lakes:
        if CoordinateIsVisible(
            HecsCoord.from_offset(lake.r, lake.c), follower_location, follower_orientation, fog_end
        ):
            location_description = DescribeLocationFromActor(
                HecsCoord.from_offset(lake.r, lake.c), follower_location, follower_orientation, map_update, cards
            )
    ...
    for mountain in metadata.mountains:
        if CoordinateIsVisible(
            HecsCoord.from_offset(mountain.r, mountain.c), follower_location, follower_orientation, fog_end
        ):
            location_description = DescribeLocationFromActor(
                HecsCoord.from_offset(mountain.r, mountain.c),
                follower_location, follower_orientation,
                map_update,
                cards,
            )
            metadata_descriptions.append(
    ...
    for city in metadata.cities:
        if CoordinateIsVisible(
            HecsCoord.from_offset(city.r, city.c), follower_location, follower_orientation, fog_end
        ):
            location_description = DescribeLocationFromActor(
                HecsCoord.from_offset(city.r, city.c), follower_location, follower_orientation, map_update, cards
            )
            metadata_descriptions.append(
    ...
    for outpost in metadata.outposts:
        if CoordinateIsVisible(
            HecsCoord.from_offset(outpost.r, outpost.c), follower_location, follower_orientation, fog_end
        ):
            location_description = DescribeLocationFromActor(
                HecsCoord.from_offset(outpost.r, outpost.c), follower_location, follower_orientation, map_update, cards
            )
    ...
    for tile in map_update.tiles:
        ...
        if tile.cell.coord == follower_forward:
            nearby_tiles.append(f"Forward tile: heading {direction:.0f} and distance {distance:.1f} {AssetId(tile.asset_id).name}")
        elif tile.cell.coord == follower_left:
            nearby_tiles.append(f"Left tile: heading {direction:.0f} and distance {distance:.1f} {AssetId(tile.asset_id).name}")
        elif tile.cell.coord == follower_right:
            nearby_tiles.append(f"Right tile: heading {direction:.0f} and distance {distance:.1f} {AssetId(tile.asset_id).name}")
    ...
    print(follower_location, follower_orientation)
